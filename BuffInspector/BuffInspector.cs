...
public class BuffInspectorPlugin(ISwiftlyCore core) : BasePlugin(core)
{
    ...
    public override void OnSharedInterfaceInjected(IInterfaceManager interfaceManager)
    {
        ...
        httpClient.DefaultRequestHeaders.Add(
            "User-Agent",
            "Mozilla/5.0 (iPhone; CPU iPhone OS 16_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) buff iPhone"
        );
    ...
    public void BuffCommand(ICommandContext context)
    {
        if (
            weaponSkinApi == null
            || !(context.Sender?.IsValid ?? false)
            || !(context.Sender?.PlayerPawn?.IsValid ?? false)
        )
        {
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                Core.Scheduler.NextTick(() =>
                {
                    ...
                    context.Reply(
                        $"» 皮肤编号:{skinInfo.PaintIndex} | 图案模板:{skinInfo.PaintSeed} | \x09{skinInfo.PaintWear:F17}"
                    );
                    ...
                    skinInfo
                        .Stickers.Where(s => s.Id != 0)
                        .Select(s =>
                            s.Id < 0
                                ? $"印花 \x0A»\x01 {s.Name} \x0F解析失败"
                                : $"印花 \x0A»\x01 {s.Name} \x05{1f - s.Wear:F4}"
                        )
                        .ToList()
                        .ForEach(context.Reply);

                    skinInfo
                        .Keychains.Where(s => s.Id != 0)
                        .Select(s =>
                            s.Id < 0
                                ? $"挂件 \x0A»\x01 {s.Name} \x0F解析失败"
                                : $"挂件 \x0A»\x01 \x0E{s.Name}"
                        )
                        .ToList()
                        .ForEach(context.Reply);
                    ...
                    switch (skinInfo.Type)
                    {
                        case SkinType.Weapon:
                            weaponSkinApi.UpdateWeaponSkin(
                                steamId,
                                team,
                                (ushort)skinInfo.DefinitionIndex,
                                skin =>
                                {
